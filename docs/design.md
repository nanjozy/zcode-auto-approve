...
| T8 | **模型抖动/幻觉放行**（v0.3 新增） | 最小安全网是代码级不变式，与模型输出解耦；单元测试锁定安全网行为 |
...
### 10.1 单元测试（`node --test`，全部 mock、绝不触网）
...
- **v0.2（2026-09-15，M2）**：纯规则实现与安全修订（见 git 历史；ADR-0003/0006 原文已并入 ADR-0007「历史决策」小节）。
